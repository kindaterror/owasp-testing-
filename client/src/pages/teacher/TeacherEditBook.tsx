...
export default function TeacherEditBook() {
  ...
  const handleAddPage = () => {
    ...
    const newPage: PageFormValues = { pageNumber: newPageNumber, title: "", content: "", imageUrl: "", questions: [] };
      ...
      >
        ...
        <div className="container mx-auto px-4 relative">
          <div className="flex items-center justify-center mb-3">
            ...
            <span className="text-sm font-heading font-bold text-ilaw-gold tracking-wide uppercase">
              Ilaw ng Bayan Learning Institute
            </span>
          </div>
          <h1 className="text-2xl md:text-3xl font-heading font-bold text-center">✏️ Edit Book</h1>
          <p className="text-blue-100 text-center">Update your educational content</p>

          {/* breadcrumbs */}
          <div className="mt-3 flex items-center justify-center text-[11px] text-blue-100/80 gap-2">
            ...
            <span>›</span>
            <span className="font-semibold text-white/90">Edit</span>
          </div>
        </div>
      </motion.div>

      <main className="flex-1 py-6 container mx-auto px-4">
        {/* Back button (keeps admin layout rhythm) */}
        <motion.div variants={fadeInFast} initial="hidden" animate="visible" className="mb-6">
          <Button
            variant="ghost"
            className="border-2 border-brand-navy-200 text-ilaw-navy hover:bg-brand-navy-50 font-heading font-bold"
        ...
        {/* Main card with the same layout as admin: left form + right cover */}
        <motion.div
          variants={fadeIn}
          initial="hidden"
          animate="visible"
        ...
        >
          <div className="border-b border-brand-navy-200 p-4 rounded-t-2xl">
            ...
            <p className="text-sm text-brand-navy-700 mt-0.5">Update book details and content</p>
          </div>
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
                <motion.div variants={stagger} initial="hidden" animate="visible" className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                  {/* LEFT (2/3): inputs, quiz, music, description, badges */}
                  <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-1.5 auto-rows-min">
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem className="!space-y-1">
                            ...
                            <FormControl>
                              <Input placeholder="Book title" className="border-2 border-brand-navy-200 focus:border-ilaw-gold" {...field} />
                            </FormControl>
                            <p className="text-[11px] text-slate-500 mt-0.5">Keep it short and descriptive.</p>
                            <FormMessage />
                          </FormItem>
                        )}
                      />
                        ...
                        render={({ field }) => (
                          <FormItem className="!space-y-1">
                            ...
                            <Select onValueChange={field.onChange} value={field.value}>
                              <FormControl>
                                <SelectTrigger className="border-2 border-brand-navy-200 focus:border-ilaw-gold">
                                  <SelectValue placeholder="Select type" />
                                </SelectTrigger>
                              </FormControl>
                              <SelectContent className="border-2 border-brand-navy-200">
                    ...
                    <AnimatePresence initial={false} mode="popLayout">
                      {form.watch("type") === "educational" && (
                        <motion.div key="subject-field" variants={itemFade} initial="hidden" animate="visible" exit="exit" className="md:col-span-2">
                          <FormField
                            control={form.control}
                            name="subject"
                            render={({ field }) => (
                              <FormItem className="!space-y-1">
                                ...
                                <p className="text-[11px] text-slate-500 mt-0.5">Required for educational books.</p>
                                <FormMessage />
                              </FormItem>
                            )}
                          />
                    ...
                    <motion.div variants={itemFade} className="md:col-span-2">
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem className="!space-y-1">
                            <FormLabel className="font-heading font-bold text-ilaw-navy">🧠 Quiz Mode</FormLabel>
                            ...
                            <p className="text-[11px] text-slate-500 mt-0.5">Controls how students can attempt the quiz.</p>
                            <FormMessage />
                          </FormItem>
                        )}
                      />
                          ...
                          <FormItem>
                            ...
                            <div className="mt-2">
                              <Input
                                ...
                                disabled={audioUploading}
                              />
                              {audioUploading && <p className="text-[11px] text-slate-500 mt-1">Uploading audio…</p>}
                            </div>
                            <FormMessage />
                          </FormItem>
                        )}
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <FormControl>
                              <Textarea placeholder="Enter book description" className="min-h-[96px] border-2 border-brand-navy-200 focus:border-ilaw-gold" {...field} />
                            </FormControl>
                            <p className="text-[11px] text-slate-500 mt-0.5">A brief overview students will see.</p>
                            <FormMessage />
                          </FormItem>
                        )}
                      />
                    ...
                    {/* Badges & Award Rules (same UI as admin) */}
                    <motion.div variants={itemFade} className="md:col-span-2 border-2 border-brand-navy-200 bg-brand-navy-50 rounded-xl p-4 mt-1.5">
                      <h3 className="text-lg font-heading font-bold mb-3 text-ilaw-navy">🏅 Badges & Award Rules</h3>
                      ...
                      {isLoadingBadges || isLoadingBookBadges ? (
                        <div className="bg-white p-4 rounded-xl text-center border-2 border-brand-navy-200">
                          ...
                          <span className="text-brand-navy-700 font-medium">Loading badges...</span>
                        </div>
                      ) : allBadges.length === 0 ? (
                        <div className="bg-white p-4 rounded-xl text-center border-2 border-brand-navy-200">
                          <p className="text-brand-navy-700 font-medium">No badges available.</p>
                        </div>
                      ) : (
                              ...
                              return (
                                ...
                                >
                                  <div className="flex items-center justify-between">
                                    <div>
                                      <p className="font-heading font-bold text-ilaw-navy">{b.name}</p>
                                      {b.description ? (
                                        <p className="text-sm text-brand-navy-700">{b.description}</p>
                                      ) : null}
                                    </div>
                                    <div className="flex items-center gap-2">
                                      ...
                                      />
                                      <span className="text-sm text-ilaw-navy font-medium">Enabled</span>
                                    </div>
                                  </div>
                                    ...
                                    {m?.isEnabled && (
                                      ...
                                      >
                                        <div className="md:col-span-1">
                                          <label className="block text-sm font-bold text-ilaw-navy mb-1">Award Method</label>
                                          <Select
                                            value={m.awardMethod}
                                        ...
                                        <div className="md:col-span-1">
                                          <label className="block text-sm font-bold text-ilaw-navy mb-1">% Completion Threshold</label>
                                          <Input
                                            type="number"
                                            min={1}
                                            max={100}
                                            ...
                                            className="border-2 border-brand-navy-200 focus:border-ilaw-gold"
                                          />
                                          <p className="text-[11px] text-brand-navy-700 mt-0.5">
                  ...
                  <motion.div variants={itemFade} className="lg:col-span-1">
                    <div className="rounded-xl border-2 border-brand-navy-200 bg-brand-navy-50 p-3 h-full">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <h4 className="font-heading font-bold text-ilaw-navy">Cover Image</h4>
                      </div>
                ...
                <motion.div variants={itemFade} initial="hidden" animate="visible" className="border-2 border-brand-navy-200 bg-brand-navy-50 rounded-xl p-4">
                  <h3 className="text-lg font-heading font-bold mb-3 text-ilaw-navy flex items-center">
                  ...
                  {pages.length === 0 ? (
                    <motion.div variants={itemFade} initial="hidden" animate="visible" className="bg-white p-6 rounded-xl text-center border-2 border-brand-navy-200">
                      <p className="text-brand-navy-700 font-medium mb-3">📝 No pages added yet</p>
                      <Button
                        type="button"
                        onClick={handleAddPage}
                        className="bg-ilaw-gold hover:bg-brand-gold-600 text-ilaw-navy font-heading font-bold transition-transform hover:-translate-y-0.5"
                      >
                    ...
                    <div className="space-y-4">
                      ...
                      <Button
                        ...
                        variant="outline"
                        onClick={handleAddPage}
                        className="w-full py-5 border-2 border-brand-navy-200 text-ilaw-navy hover:bg-brand-navy-50 font-heading font-bold transition-transform hover:-translate-y-0.5"
                      >
          ...
          <div className="container max-w-5xl mx-auto">
            <div className="rounded-xl border-2 border-brand-navy-200 bg-white/90 backdrop-blur p-3 shadow-lg flex items-center justify-end gap-3">
              ...
              <Button
                ...
                disabled={isSaving}
                className="min-w-[140px] bg-gradient-to-r from-brand-gold-500 to-ilaw-gold hover:from-ilaw-gold hover:to-brand-gold-600 text-ilaw-navy font-heading font-bold"
              >
